...
$(document).bind("mobileinit", function(){
  $.mobile.defaultPageTransition='none';
	...
	sdate=searchValue.match(/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/);
	stime=searchValue.match(/[0-9][0-9]:[0-9][0-9]/g);
	if (stime && sdate) {
		searchTime=true;
		var sdate=sdate[0].split('-');
		var sday=parseInt(sdate[2],10);
		sstart=stime[0].split(':');
		sstart=sday*1440+parseInt(sstart[0],10)*60+parseInt(sstart[1],10);
		var edate=text.match(/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/);
		var etime=text.match(/[0-9][0-9]:[0-9][0-9]/g);

		var edate=edate[0].split('-');
		var eday=parseInt(edate[2],10);
		var estart=etime[0].split(':');
		var eend=etime[1].split(':');
		var estart=eday*1440+parseInt(estart[0],10)*60+parseInt(estart[1],10);
		var eend=eday*1440+parseInt(eend[0],10)*60+parseInt(eend[1],10);
	}

        if (filterCallbackTimeout) {
		clearTimeout(filterCallbackTimeout);
	}
	filterCallbackTimeout=setTimeout(function(){
		filterCallbackTimeout=null;
		updateNothing();
	},100);
	return searchTime?!(sstart>=estart && sstart<=eend):filterCallback(text.toLowerCase(),searchValue);
    }
